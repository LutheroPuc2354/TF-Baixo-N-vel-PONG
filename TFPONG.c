...
int main(int argc, char* argv[]) {
    ...
    SDL_Rect botaoDif1 = {LARGURA/2 - 150, ALTURA/2 + 40, 150, 60};
    SDL_Rect botaoDif2 = {LARGURA/2 + 30, ALTURA/2 + 40, 150, 60};
    SDL_Rect botaoJogarNovamente = { LARGURA/2 - 100, ALTURA/2 + 80, 200, 60 };
    ...
    while (1) {
        while (SDL_PollEvent(&e)) {
            ...
            if (estado == SELECIONANDO_MODO && e.type == SDL_MOUSEBUTTONDOWN) {
                ...
                if (x >= botaoPvP.x && x <= botaoPvP.x + botaoPvP.w &&
                    ...
                    modoCPU = false;
                    estado = SELECIONANDO_DIFICULDADE;
                }
            ...
            if (estado == FIM_DE_JOGO && e.type == SDL_MOUSEBUTTONDOWN) {
                int x = e.button.x;
                int y = e.button.y;
        ...
        if (estado == SELECIONANDO_MODO) {
            ...
            SDL_Rect r1 = {botaoPvP.x + (botaoPvP.w - s1->w)/2, botaoPvP.y + (botaoPvP.h - s1->h)/2, s1->w, s1->h};
            SDL_Rect r2 = {botaoCPU.x + (botaoCPU.w - s2->w)/2, botaoCPU.y + (botaoCPU.h - s2->h)/2, s2->w, s2->h};
        ...
        if (estado == SELECIONANDO_DIFICULDADE) {
            ...
            SDL_Texture* t1 = SDL_CreateTextureFromSurface(render, s1);
            SDL_Texture* t2 = SDL_CreateTextureFromSurface(render, s2);
            SDL_Rect r1 = {botaoDif1.x + (botaoDif1.w - s1->w)/2, botaoDif1.y + (botaoDif1.h - s1->h)/2, s1->w, s1->h};
            SDL_Rect r2 = {botaoDif2.x + (botaoDif2.w - s2->w)/2, botaoDif2.y + (botaoDif2.h - s2->h)/2, s2->w, s2->h};
            SDL_RenderCopy(render, t1, NULL, &r1);
            SDL_RenderCopy(render, t2, NULL, &r2);
            SDL_FreeSurface(s1); SDL_FreeSurface(s2);
            SDL_DestroyTexture(t1); SDL_DestroyTexture(t2);
